...
export default function BannerAdmin() {
  ...
  const fetchData = async () => {
    try {
      ...
      const docs = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Banner));
      setData(docs);
    } catch (error) {
      console.error(error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        const docRef = doc(db, 'banners', formData.id);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Kelola Pengaturan Banner</h1>
        <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 shrink-0">
              <h3 className="text-lg font-bold text-gray-900">{isEditing ? 'Edit Banner' : 'Tambah Banner'}</h3>
