...
export default function UserAdmin() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const q = query(collection(db, 'kemenag_allowed_users'), orderBy('createdAt', 'desc'));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        ...
        const docRef = doc(db, 'kemenag_allowed_users', editingUser.id);
        ...
        // Add new user
        await addDoc(collection(db, 'kemenag_allowed_users'), {
          email: normalizedEmail,
          name: formData.name.trim(),
          role: formData.role,
  ...
  const handleDelete = async (id: string, email: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'kemenag_allowed_users', id));
